...
String::String(const_pointer _str) {
  if (_str != nullptr) {
    ...
    throw StringException("String equals nullptr, ::String(const char *_str)");
  }
}
...
String::String(size_type i) { m_len = i; }
...
String &String::append(const_pointer _str) {
  if (_str != nullptr) {
    ...
    throw StringException("String equals nullptr, ::append(const char *_str)");
  }
...
String &String::replace(size_type _pos, size_type _size, const_pointer _str) {
  if (!_str) {
    throw StringException("::Replace(...) got nullptr string");
  }

  char *_buffer = new char[_size + 1];

  for (size_type i = 0; i < _size; i++) {
    _buffer[i] = _str[i + _pos];
  }
  ...
  try {
    ...
    std::cerr << err_alloc.what() << std::endl;
  }

  strncpy(m_str, _buffer, _size);
...
const char &String::back() const { return operator[](m_len - 1); }

char &String::back() { return operator[](m_len - 1); }

const char &String::front() const { return operator[](0); }
...
const char *String::c_str() const noexcept { return m_str; }

const char *String::data() const noexcept { return m_str; }
...
String::Iterator String::begin() const { return f_begin(); }

String::Iterator String::end() const { return l_end(); }
...
std::size_t String::size() const noexcept { return m_len; }

std::size_t String::max_size() const noexcept { return String::npos; }

std::size_t String::length() const noexcept { return m_len; }
...
bool String::empty() const noexcept { return m_len ? false : true; }

int String::compare(const_pointer _str) const {
  if (!_str) {
    throw StringException("String equals nullptr, ::compare(const char *_str)");
  }
...
void String::swap(String &_rhs) noexcept {
  std::swap(m_str, _rhs.m_str);
  std::swap(m_len, _rhs.m_len);
  std::swap(m_len, _rhs.m_len);
}

void String::swap(String &_rhs, String &_lhs) noexcept { _rhs.swap(_lhs); }
...
void String::reserve(size_type new_cap) {
  if (new_cap > m_cap) {
    ...
    throw std::runtime_error(
        "Capacity size corresponds to the original size,let's do more");
  }
}

void String::t_substr(char *_buffer, const char *t_str, size_type _pos,
